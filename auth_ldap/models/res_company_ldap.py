...
class CompanyLDAP(models.Model):
    ...
    def authenticate(self, conf, login, password):
        ...
        """
        attrs = ['cn']
        filtro = 'userPrincipalName='+login
        ...
        try:
            # Get rid of (None, attrs) for searchResultReference replies
                results = self.query(conf, filtro)
                conn = self.connect(conf)
                print(login)
                print(password)
                print(conf['ldap_base'])
                print(filtro)
                conn.simple_bind_s(login, password)
                print('paso1')
                result= str(conn.search_s(conf['ldap_base'], ldap.SCOPE_SUBTREE, filtro, attrs)[0][1]['cn'])
                print(str(result))
                entry = str(result)
                print('paso2')
                conn.unbind()
    ...
    def query(self, conf, filter, retrieve_attributes=None):
        ...
        try:
            ...
            #print str(conf['ldap_base'].encode('utf-8'))+str(ldap.SCOPE_SUBTREE)+ str(filter)+ str(retrieve_attributes)
            print(filter)
            results = conn.search_s(conf['ldap_base'], ldap.SCOPE_SUBTREE, filter, retrieve_attributes)[0][1]['cn']
            conn.unbind()
        except ldap.INVALID_CREDENTIALS:
